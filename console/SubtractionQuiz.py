from mindcalc.quiz import SubtractionQuiz, Difficulty
import time
...
def main():
    ...
    if difficulty == 1:
        quiz = SubtractionQuiz(3, 2, Difficulty.EASY)
    elif difficulty == 2:
        quiz = SubtractionQuiz(5, 2, Difficulty.MEDIUM)
    else:  # defaults to HARD if other values are entered
        quiz = SubtractionQuiz(10, 2, Difficulty.HARD)
